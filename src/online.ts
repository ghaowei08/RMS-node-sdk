...
interface RazerOnlineSDKInstance {
	...
	getTransactionsByDay: (T: Date) => Promise<PaymentTransactionRes[]>,
	...
	getSettlements: (T: SettlementReq) => Promise<SettlementRes[]>
...
export class RazerOnlineSDK implements RazerOnlineSDKInstance {
	...
	async getTransactionsByDate({
		...
		const duration = moment(end).diff(moment(start), 'day') + 1;
		const result = await Promise.all<PaymentTransactionRes[]>(new Array(duration).fill(null).map((e, i) => this.getTransactionsByDay(moment(start).add(i, 'day').toDate())));
		return result.flatMap(e => e);
	}

	async getTransactionsByDay(day: Date): Promise<PaymentTransactionRes[]> {
		const toDate = moment(day).format('YYYY-MM-DD HH:mm:ss');
		...
		const response = await this.apiInstance({
			url: `/API/PSQ/psq-daily.php?merchantID=${this.merchantId
					...
					key,
				)}&rdate=${toDate}&version=4.0&response_type=json&additional_fields=CardScheme,CardType,CardCountry,TransactionRate,TransactionFee,NetAmount,PaidDate,RefundRefID`,
			method: 'GET',
		});
		...
		return res.data?.reduce((pv: SettlementRes[], curr: SettlementSummary | SettlementRecord) => {
			switch (curr.RecordIdentifier) {
				...
				case SETTLEMENT_IDENTIFIER.R:
					...
					break;
				default:
					throw 'SETTLEMENT_IDENTIFIER NOT FOUND';
			}
			return pv;
		}, []);
	}
